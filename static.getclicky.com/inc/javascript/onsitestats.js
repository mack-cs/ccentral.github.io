var _OSS = {
	...
	heatmap: function( sub, date, subitem ) {
		...
		_OSS.secondary_loading();
		_cgen.heatmap( _OSS.heatmap_date, sub, subitem );
	},
	...
	visitors_load: function( page, href, sub ) {
		...
		if( href && typeof href != 'string' ) href = _cgen.get_href();
		
		_cgen.inject( _OSS.hq + '/ajax/onsitestats/visitors?site_id=' + _OSS.site_id + ( _OSS.sitekey ? '&sitekey=' + _OSS.sitekey : '' ) + '&page=' + ( page||1 ) + ( href ? '&href=' + href + '&domain=' + location.hostname : '' ) + ( sub ? '&sub=' + sub : '' ) + '&x=' + Math.random());
	},
